					...
					listeners:{
						swipe:{
							fn:function(event){
								...
								var panel = Ext.getCmp("articleContent");
								...
								if(event.direction == "left"){ //다음 기사로 이동
									if(flag==count-1){
										Ext.Msg.alert("알림", "다음 기사가 없습니다.", Ext.getCmp("mainPanel"));
									}else{
										panel.setData(data.items[++flag].data);
										localStorage.flag = flag;
										Ext.Anim.run(Ext.getCmp("articleContent"), 'slide', {
											out:false,
											autoClear:false,
										});
									}
									...
									if(flag==0){
										Ext.Msg.alert("알림", "이전 기사가 없습니다.", Ext.getCmp("mainPanel"));
									}else{
										panel.setData(data.items[--flag].data);
										localStorage.flag = flag;
										Ext.Anim.run(Ext.getCmp("articleContent"), 'slide', {
