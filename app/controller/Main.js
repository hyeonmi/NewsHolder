Ext
		.define(
				'NewsHolder.controller.Main',
				{
					extend : 'Ext.app.Controller',
					requires : [ 'Ext.data.proxy.JsonP',
							'Ext.data.proxy.LocalStorage', 'Ext.TitleBar',
							'Ext.dataview.DataView', 'Ext.dataview.List',
					...
					articleScrapButtonTap : function(button, event) {
						...
						if (localStorage.getItem("scrap-counter") == null) { // 스크랩을
							// 처음으로
							// 저정할
							// 때,
							...
							thirdString = localStorage.getItem("scrap") + ","
									+ id;
						}
						...
						// link:
						// this.getArticle().items.items[0].items.items[1]._data.link,
						// scrapDate: 스크랩한 시간,
						};

						localStorage.setItem("scrap-" + id, JSON
								.stringify(data));
					...
					/** 기사 리스트에서 기사를 눌렀을 때 */
					onArticleTap : function(dataview, index, target, record, e,
							options) {
					...
					font_size_up : function(button, event) {
						var current = parseInt($("#mainArticle").css(
								"font-size"));
						console.log(current);
