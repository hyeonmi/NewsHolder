Ext.define('NewsHolder.controller.KeywordSearchController', {
    ...
    onSearchButtonTap: function(button, e, options) {
        ...
        var naverUrl="http://openapi.naver.com/search?key=907ed5cd8ab441dc61ffb0016280e906&query="+encodedInput+"&target=news";
        var encodedNaverUrl=encodeURIComponent(naverUrl);
        var feedUrl="http://iamapark.cafe24.com/fullrss/makefulltextfeed.php?url="+encodedNaverUrl+"&format=json";
    ...
    onSearchListItemTap: function(dataview, index, target, record, e, options) {
        /**
        list 하나로 두 개의 store가 같이 사용하고 있다. 
        그러므로 store가 실시간 검색어 순위 store일 때와검색결과 store일 때 
        tap 이벤트를 다르게 처리해야 한다.
        if문에는 검색결과 store와 현재 리스트에 연결되어있는 store가 같은 지를 판별하는
        문장이다. 같으면 데이터를 연결해주고 기사를 보여준다.
        else문에는 검색결과 store와 현재 리스트에 연결되어있는 store가 다른 경우이다.
        다른 경우에는 list에서 선택된 검색어로 네이버 뉴스 검색을 하여 list에 나타낸다.
        */
